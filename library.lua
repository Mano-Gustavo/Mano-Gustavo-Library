--[[ 
    🌟 MANO GUSTAVO UI - DIAMOND EDITION (V7.8 - Hosted Module)
    Esta é a versão MÓDULO. Ela deve ser carregada via loadstring.
    Não contém exemplos de execução, apenas a estrutura da biblioteca.
]]

return (function()
    --// CONFIGURAÇÕES GERAIS E SERVIÇOS //--
    ...
    local GuiName = "GustavoHub_Hosted_V7"
    ...
    function Lib:Notification(Config)
        ...
        IconImg.Parent = Frame; IconImg.BackgroundTransparency = 1; IconImg.Position = UDim2.new(0, 10, 0, 10)
        IconImg.Size = UDim2.new(0, 24, 0, 24); IconImg.Image = SelectedType.Icon; IconImg.ImageColor3 = SelectedType.Color

        local T_Title = Instance.new("TextLabel")
        T_Title.Parent = Frame; T_Title.BackgroundTransparency = 1; T_Title.Text = Title; T_Title.TextColor3 = SelectedType.Color
        T_Title.Font = Enum.Font.GothamBold; T_Title.TextSize = 14; T_Title.Position = UDim2.new(0, 42, 0, 12); T_Title.Size = UDim2.new(1, -50, 0, 20); T_Title.TextXAlignment = 0

        local T_Text = Instance.new("TextLabel")
        T_Text.Parent = Frame; T_Text.BackgroundTransparency = 1; T_Text.Text = Text; T_Text.TextColor3 = UIConfig.TextColor
        T_Text.Font = Enum.Font.Gotham; T_Text.TextSize = 12; T_Text.Position = UDim2.new(0, 42, 0, 32); T_Text.Size = UDim2.new(1, -50, 0, 30); T_Text.TextXAlignment = 0; T_Text.TextWrapped = true
        ...
        task.delay(Duration, function()
            local tw = TweenService:Create(Frame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Size = UDim2.new(1, 100, 0, 0), Transparency = 1})
            tw:Play()
            tw.Completed:Wait()
            Frame:Destroy()
        end)
    ...
    function Lib:CreateWindow(Config)
        ...
        CreateInstance("UICorner", {Parent = SearchBarBG, CornerRadius = UDim.new(0, 5)}); CreateInstance("UIStroke", {Parent = SearchBarBG, Color = UIConfig.StrokeColor, Thickness = 1})
        ...
        -- Definições de Container (Scopes)
        local PageHolder -- Definido aqui para ser acessível pela Search Logic
            ...
            if ActivePage then
                for _, item in pairs(ActivePage:GetChildren()) do
                    if item:IsA("Frame") or item:IsA("TextButton") then
                        -- Busca profunda em TextLabels dentro dos botões
                        local foundText = false
                        local descendants = item:GetDescendants()
                        -- Verifica o proprio objeto se tiver text property
                        if item:IsA("TextButton") and string.find(item.Text:lower(), text) then foundText = true end

                        for _, d in pairs(descendants) do
                            if d:IsA("TextLabel") and string.find(d.Text:lower(), text) then
                                foundText = true
                                break
                            end
                        end
                        item.Visible = foundText
                    end
                end
            end
        end
        ...
        UserInputService.InputBegan:Connect(function(input, gp)
            if not gp and input.KeyCode == OpenKey then ToggleUI(UI_Hidden) end
        end)

        -- Conteúdo da Window (Tabs e Pages)
        ...
        -- Inicialização real do PageHolder
        ...
        function Tabs:CreateTab(Name)
            ...
            local function Activate()
                if SearchOpen then SearchInput.Text = ""; DoSearch("") end -- Reset search
            ...
            function Elems:CreateButton(Txt, Callback)
                ...
                AddHover(Btn); Btn.MouseButton1Click:Connect(function() pcall(Callback) end)
            end
            ...
            function Elems:CreateColorPicker(Txt, Default, Callback)
                Order += 1
                local Expanded, MinH, MaxH = false, 34, 140
                ...
                local ColorPreview = CreateInstance("Frame", {Parent = HeaderBtn, BackgroundColor3 = CurColor, Size = UDim2.new(0,35,0,20), Position = UDim2.new(1,-45,0.5,-10)}); CreateInstance("UICorner", {Parent = ColorPreview, CornerRadius = UDim.new(0,4)})
                ...
                local function UpdateColor()
                    ...
                    ColorPreview.BackgroundColor3 = CurColor
                ...
                local function CreateRGBItem(Name, Col, MaxVal, YPos)
                    ...
                    CreateInstance("TextLabel", {Parent = SlidersFrame, BackgroundTransparency = 1, Text = Name, TextColor3 = Col, Position = UDim2.new(0,10,0,YPos), Font = Enum.Font.GothamBold, TextSize = 12, Size = UDim2.new(0,20,0,20)})
                    ...
                    UserInputService.InputChanged:Connect(function(i)
                        if drag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
                            local sc = math.clamp((i.Position.X - BarBG.AbsolutePosition.X)/BarBG.AbsoluteSize.X, 0, 1)
                            local val = math.floor(sc * 255)
                            ValLab.Text = tostring(val)
                            TweenService:Create(Fill, TweenInfo.new(0.05), {Size = UDim2.new(sc,0,1,0)}):Play()
                ...
                CreateRGBItem("R", Color3.fromRGB(255,80,80), R, 10); CreateRGBItem("G", Color3.fromRGB(80,255,80), G, 40); CreateRGBItem("B", Color3.fromRGB(80,80,255), B, 70)
                HeaderBtn.MouseButton1Click:Connect(function() Expanded = not Expanded; TweenService:Create(Container, TweenInfo.new(0.2), {Size = UDim2.new(1,0,0, Expanded and MaxH or MinH)}):Play() end)
            end

            function Elems:CreateDropdown(Txt, Options, Callback)
                Order += 1
                local Expanded, DropH = false, 34
                ...
                HeadBtn.MouseButton1Click:Connect(function()
                    ...
                    local CS = ListLayout.AbsoluteContentSize.Y + 40
                    TweenService:Create(Container, TweenInfo.new(0.2), {Size = UDim2.new(1,0,0, Expanded and CS or DropH)}):Play()
                    TweenService:Create(Arrow, TweenInfo.new(0.2), {Rotation = Expanded and 180 or 0}):Play()
                    TweenService:Create(ListFrame, TweenInfo.new(0.2), {Size = UDim2.new(1,0,0, Expanded and (CS-34) or 0)}):Play()
                end)
                ...
                for _, opt in pairs(Options) do
                    ...
                    OptBtn.MouseButton1Click:Connect(function()
                        Expanded = false; Label.Text = Txt .. ": " .. opt; pcall(Callback, opt)
            ...
            function Elems:CreateTextBox(Txt, Placeholder, Callback)
                ...
                local InputBG = CreateInstance("Frame", {Parent = BoxFrame, BackgroundColor3 = Color3.fromRGB(25,25,25), Size = UDim2.new(1,-20,0,24), Position = UDim2.new(0,10,0,25), BackgroundTransparency = 0.2}); CreateInstance("UICorner", {Parent = InputBG, CornerRadius = UDim.new(0,4)})
            ...
            function Elems:CreateKeybind(Txt, DefKey, Callback)
                ...
                local BindBtn = CreateInstance("TextButton", {Parent = BindFrame, BackgroundColor3 = Color3.fromRGB(50,50,50), Size = UDim2.new(0,80,0,22), Position = UDim2.new(1,-90,0.5,-11), Text = Key.Name, Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = Color3.new(0.9,0.9,0.9), AutoButtonColor = false}); CreateInstance("UICorner", {Parent = BindBtn, CornerRadius = UDim.new(0,4)})
        ...
        return Tabs -- Retorna a tabela de abas corretamente
    end
    
    return Lib -- Retorno Final da Library para o Loader
end)()
